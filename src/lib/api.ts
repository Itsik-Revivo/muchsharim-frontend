...
export function setToken(token: string) {
  localStorage.setItem('muchsharim_token', token);
}

export function clearToken() {
  localStorage.removeItem('muchsharim_token');
  localStorage.removeItem('muchsharim_user');
}
...
// Waxman projects
export const getWaxmanProjects = (search?: string, domain?: string) => {
  const qs = new URLSearchParams();
  if (search) qs.set('search', search);
  if (domain) qs.set('domain', domain);
  return api.get(`/api/waxman-projects${qs.toString() ? '?' + qs : ''}`);
};
export const getWaxmanProject = (id: string) => api.get(`/api/waxman-projects/${id}`);
